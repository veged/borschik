...
var freeze = exports.freeze = function(filePath, content) {
    ...
    if (_freezeDir) {
        ...
        filePath = _freezeDir + PATH.sep + hash + PATH.extname(filePath);
...
var loadConfig = exports.loadConfig = function(path) {
    if (configs.paths[path] === undefined) {
        ...
        if (FS.existsSync(path + PATH.sep + '.borschik')) {
            configs.paths[path] = true;
            
            var _config = JSON.parse(FS.readFileSync(path + PATH.sep + '.borschik'));
...
var realpathSync = exports.realpathSync = function(path) {
    ...
    for (var i = 0; i < folders.length; ) {
        ...
        for (var j = 0; j < i; j++) {
            var subprefix = subArrayJoin(folders, PATH.sep, 0, j);
            var followSymlinksJ = followSymlinks(subprefix);
            if (followSymlinksJ !== undefined) _followSymlinks = followSymlinksJ;
        }

        if (_followSymlinks && isSymLink(prefix + PATH.sep + name)) {
            var link = FS.readlinkSync(prefix + PATH.sep + name);
            var linkParts = link.split(PATH.sep);
            linkParts.shift();
            if (new RegExp('^' + PATH.sep).test(link)) {
                linkParts.shift();
